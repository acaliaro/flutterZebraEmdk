...
public class MainActivity extends FlutterActivity implements EMDKListener,
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (results.statusCode != EMDKResults.STATUS_CODE.SUCCESS) {
            Log.d(TAG, "EMDKManager Request Failed");
        }

        new EventChannel(getFlutterEngine().getDartExecutor(), BARCODE_RECEIVED_CHANNEL).setStreamHandler(
                new StreamHandler() {
                    ...
                    public void onListen(Object arguments, EventSink events) {
                        Log.d(TAG, "EventChannelOnListen");
                    ...
                    public void onCancel(Object arguments) {
                        Log.d(TAG, "EventChannelOnCancel");
    ...
    private BroadcastReceiver createBarcodeBroadcastReceiver(final EventSink events) {
        return new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                Log.d(TAG, "createBarcodeBroadcastReceiver " + action);

                if (action.equals("readBarcode")) {
                    String barcode = intent.getStringExtra("barcode");
                    String barcodetype = intent.getStringExtra("barcodetype");
                    Log.d(TAG, "createBarcodeBroadcastReceiver " + barcode);
    ...
    private void enumerateScannerDevices() {
        ...
        Log.d(TAG, "snumerateScannerDevices()");
...
/*
    ...
    public void onData(ScanDataCollection scanDataCollection) {
        ...
        if ((scanDataCollection != null) && (scanDataCollection.getResult() == ScannerResults.SUCCESS)) {
            ...
            for (ScanDataCollection.ScanData data : scanData) {
                ...
                try {
                    Log.d(TAG, data.getLabelType() + "-" + data.getData());
    ...
    public void onStatus(StatusData statusData) {
        ...
        Log.d(TAG, "onStatus() " + statusData.getFriendlyName());
...
/*
    ...
    private void setDecoders() {
        ...
        if ((scanner != null) && (scanner.isEnabled())) {
            try {
                ...
                Toast.makeText(this, "Status: " + e.getMessage(), Toast.LENGTH_LONG).show();
            }
        }
    }
        ...
        if (scanner != null) {
            try {
                ...
                if (scanner.isEnabled()) {
                    ...
                    Toast.makeText(this, "Status: Scanner is not enabled", Toast.LENGTH_LONG).show();
                }
    ...
    private void initScanner() {
        ...
        if (scanner == null) {
            ...
            if ((deviceList != null) && (deviceList.size() != 0)) {
                int barcodeImagedIdx = -1;
                int pos = 0;
                for (ScannerInfo device : deviceList
                ) {
                    Log.d(TAG, "scanner: " + device.getFriendlyName());
                    if(device.getFriendlyName().startsWith("2D")) // I work with 2D scanner...
                        barcodeImagedIdx = pos;
                    pos++;

                }

                scanner = barcodeManager.getDevice(barcodeImagedIdx != -1 ? deviceList.get(barcodeImagedIdx) : deviceList.get(0));

            } else {
                Toast.makeText(this, "Status: " + "Failed to get the specified scanner device! Please close and restart the application.", Toast.LENGTH_LONG).show();
                return;
            }
            ...
            if (scanner != null) {
                ...
                Toast.makeText(this, "Status: " + "Failed to initialize the scanner device.", Toast.LENGTH_LONG).show();
            }
        }
    }
